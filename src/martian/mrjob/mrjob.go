...
func (self *runner) setRlimit() {
	...
	if err := syscall.Getrlimit(syscall.RLIMIT_NOFILE, &rlim); err != nil {
		core.PrintError(err, "monitor", "Error getting file rlimit.")
		return
	}
	rlim.Cur = rlim.Max
	if err := syscall.Setrlimit(syscall.RLIMIT_NOFILE, &rlim); err != nil {
		core.PrintError(err, "monitor",
			"Could not increase the file handle ulimit to %d", rlim.Max)
	}
}

func (self *runner) done() {
	core.LogInfo("time", "__end__")
	...
	if self.jobInfo != nil {
		...
		self.jobInfo.WallClockInfo = &core.WallClockInfo{
			Start:    self.start.Format(core.TIMEFMT),
			End:      end.Format(core.TIMEFMT),
		...
		if err := self.metadata.WriteAtomic(core.JobInfoFile, self.jobInfo); err != nil {
			core.PrintError(err, "monitor", "Could not write final jobInfo.")
		} else {
...
func (self *runner) Fail(err error, message string) {
	...
	if writeError := self.metadata.WriteRaw(target, errStr); writeError != nil {
		core.PrintError(writeError, "monitor", "Could not write errors file.")
	}
	if jErr := self.metadata.UpdateJournal(target); jErr != nil {
		core.PrintError(jErr, "monitor", "Could not update error journal file.")
	}
	os.Exit(0)
}
...
func (self *runner) Complete() {
	...
	if writeError := self.metadata.WriteTime(core.CompleteFile); writeError != nil {
		core.PrintError(writeError, "monitor", "Could not write complete file.")
	}
	if jErr := self.metadata.UpdateJournal(core.CompleteFile); jErr != nil {
		core.PrintError(jErr, "monitor", "Could not update complete journal file.")
	}
	os.Exit(0)
}
...
func (self *runner) StartJob(args []string) error {
	...
	if err := self.startProfile(); err != nil {
		core.PrintError(err, "monitor", "Could not start profiling.")
	}
	return nil
}
...
func (self *runner) HandleSignal(sig os.Signal) {
	core.PrintInfo("monitor", "Caught signal %v", sig)
	cmd := self.job
	if cmd != nil {
		proc := cmd.Process
		if proc != nil {
	...
	if err := self.metadata.WriteRaw(core.Errors, fmt.Sprintf("Caught signal %v", sig)); err != nil {
		core.PrintError(err, "monitor", "Could not write errors file.")
	}
	if err := self.metadata.UpdateJournal(core.Errors); err != nil {
		core.PrintError(err, "monitor", "Could not update error journal file.")
	}
}
...
func (self *runner) monitor() error {
	...
	if err := self.metadata.UpdateJournal(core.Heartbeat); err != nil {
		core.PrintError(err, "monitor", "Could not write heartbeat.")
	}
	return nil
}
